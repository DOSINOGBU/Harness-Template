...
| 5. Plan | 모호한 부분과 가정을 표시한 실행 계획을 만듭니다. | `docs/exec-plans/active/` 또는 짧은 메모 |
| 6. Quality Gate | 핵심 산출물이 다른 기능의 입력이면 품질 기준과 승인 기준을 먼저 확인합니다. | 품질 게이트 기준 |
...
| 의도 | 흐름 | 확인할 기준 |
|---|---|---|
| Build | 맥락 확인 → 최소 계획 → 품질 게이트 확인 → 구현 → 파이프라인 연결 검증 → 제품 품질 검증 | 로딩, 빈 상태, 오류 상태, 입력 검증, 필요한 로그, 대표 샘플 품질 |
