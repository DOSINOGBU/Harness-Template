...
6. 완료 후 `docs/exec-plans/completed/`로 이동합니다.

## Plan Creation

새 기능 plan을 만들기 전에 `docs/exec-plans/active/`와 `docs/exec-plans/completed/`의 기존 plan 내용을 먼저 확인합니다.
...
| `Completed` | 완료 이동 기준을 만족했고 `docs/exec-plans/completed/`로 옮길 수 있습니다. |
...
- 품질 미달이면 기능 작업을 completed로 옮기더라도 후속 `quality-repair` 계획을 `docs/exec-plans/active/`에 반드시 만듭니다.
...
- 기능 동작이 바뀐 Feature ID는 `PRD Feature Contract`의 현재 기대 동작, 회귀 시나리오, 계약 상태가 최신입니다. (기능·계약 정의: [docs/PRODUCT_CONTEXT.md](../PRODUCT_CONTEXT.md), 회귀·검증 기준: [docs/TESTING.md](../TESTING.md))
...
- 남은 후속 작업은 별도 active plan 또는 [`docs/exec-plans/tech-debt-tracker.md`](tech-debt-tracker.md)에 기록되어 있습니다.

## Empty Active Flow

`docs/exec-plans/active/`에 실행 계획이 없으면 아래 순서로 다음 단계를 판단합니다.

1. 최근 사용자 요청, 실패한 검증, 미완료 run log를 확인합니다.
2. [`docs/exec-plans/tech-debt-tracker.md`](tech-debt-tracker.md)에서 즉시 실행 가능한 항목이 있는지 확인합니다.
3. `scripts/validate-harness.ps1 -Maintenance` warning 중 계획이 필요한 항목을 확인합니다.
4. 실행 가능한 작업이 있으면 `docs/exec-plans/active/`에 새 plan을 만들고 상태를 `Ready`로 둡니다.
...
Maintenance 검증은 `docs/exec-plans/active/`와 `docs/exec-plans/completed/`의 plan이 이 구조를 지키는지 확인합니다.
