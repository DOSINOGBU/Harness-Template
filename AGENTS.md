...
4. **실행 버튼 의무**: `리스크와 다음 판단`·`추천 작업`(로드맵) 위젯의 각 안건에는 **반드시 실행 버튼을 넣습니다** — `<button onclick="sendPrompt('구체적 지시문')">라벨 ↗</button>` 형태로, 클릭하면 해당 지시가 채팅에 그대로 입력됩니다. 버튼 없는 안건 나열은 미완성 보고입니다.
