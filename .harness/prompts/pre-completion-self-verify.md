...
24. `리스크와 다음 판단`과 `추천 작업`(다음 할 일 제안·우선순위)도 텍스트 나열로 끝내지 말고 각각 비쥬얼라이즈 위젯으로 표현해줘.
25. 검증이 있는 작업이면 검증 명령과 실제 출력을 `scripts/new-artifact.ps1 -Type validation`으로 만든 증거 파일에 기록하고, 완료 보고 **마지막 줄**을 `EVIDENCE_RECORDED: <저장소 상대 경로>`로 끝내줘. 문서만 바꾼 작업이면 `EVIDENCE_RECORDED: N/A(docs-only)`로 끝내줘.
26. 보고 언어를 점검해줘: 모든 위젯에 한 줄 제목이 있는지, 어려운 용어를 쉬운 말로 바꾸고 남은 전문 용어·변수명에 괄호 한글 병기를 했는지, `.harness/reporting.json`의 현재 비유를 사용했는지 확인해줘. 비유는 사용자가 바꿔 달라고 하기 전까지 바꾸지 마.
27. `리스크와 다음 판단`·`추천 작업` 위젯의 각 안건에 실행 버튼(sendPrompt, 라벨 끝 ↗)이 있는지, `reporting.json`의 `styleGuide` 가이드가 지정된 프로젝트라면 그 골격·언어 규율(비유→용어→왜→숫자, 정직 콜아웃)을 따랐는지 확인해줘.
