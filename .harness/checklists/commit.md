...
상세 기준은 `docs/VERSION_CONTROL.md`를 따릅니다.

- [ ] 변경 범위가 하나의 목적에 맞습니다.
- [ ] 기능 추가, 버그 수정, 리팩터링, 문서, 설정 변경이 섞이지 않았습니다.
- [ ] 자동 포맷 변경과 기능 변경을 분리했습니다.
- [ ] 앱·빌드·테스트 등 작업에 맞는 검증을 통과한 실행 가능한 상태에서만 커밋합니다.
...
- [ ] 자동 커밋을 쓰는 경우 검증 상태에 따라 `docs/VERSION_CONTROL.md` 정책을 따릅니다 (`Passed`만 코드/테스트 커밋 허용, `Partial`은 실행 계획·validation 기록만, `Failed`는 자동 커밋 없음).
- [ ] 사용자가 커밋을 명시적으로 거부했다면 추천 결과와 무관하게 커밋하지 않았습니다.
