...
모든 항목이 이번 변경에 해당하지 않으면 해당 없음(N/A)과 한 줄 근거를 남깁니다. 실행 계획 규칙·관계는 `docs/exec-plans/README.md`를 먼저 확인합니다.

- [ ] 새로 만들기 전에 모듈 창고 카탈로그(`modules/README.md`, 있는 경우)를 먼저 검색했습니다 — 이미 있는 모듈로 해결되면 재구현하지 않습니다.
- [ ] 범위가 큰 신규 기능이면 본관 코드에 바로 만들지 않고 `docs/MODULES.md`의 인큐베이터 절차로 시작했습니다(해당 없으면 N/A).
...
- [ ] UI·스타일 파일을 수정했다면 수정 전에 `docs/UI_RULES.md`를 읽었고, 토큰·공유 컴포넌트·간격 규칙을 따랐으며, 요청 범위 밖 화면을 재스타일하지 않았습니다.
